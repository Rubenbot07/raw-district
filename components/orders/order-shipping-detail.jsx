import { useTranslations } from "next-intl";
export const OrderShippingDetail = ({ address }) => {
  const shipping = address?.[0];

  const tPurchaseForm = useTranslations("PurchaseForm");
  const tOrders = useTranslations("Orders");  
  const tCommon = useTranslations("Common");

  if (!shipping) {
    return (
      <div className="bg-gray-100 rounded-[8px] p-4">
        <h2 className="text-base font-semibold mb-2">{tOrders("shippingDetails")}</h2>
        <p className="text-xs text-gray-600">{tOrders("noShippingDetails")}</p>
      </div>
    );
  }

  return (
    <section
      aria-labelledby="shipping-details-title"
      className="bg-gray-100 rounded-[8px] p-4"
    >
      <h2 id="shipping-details-title" className="text-base font-semibold mb-2">
        {tOrders("shippingDetails")}
      </h2>

      <dl className="text-xs text-gray-700 space-y-1">
        <div>
          <dt className="inline font-medium">{tPurchaseForm("fullName")}: </dt>
          <dd className="inline">
            {shipping.first_name} {shipping.last_name}
          </dd>
        </div>

        <div>
          <dt className="inline font-medium">{tPurchaseForm("address")}: </dt>
          <dd className="inline">{shipping.address_line}</dd>
        </div>

        <div>
          <dt className="inline font-medium">{tPurchaseForm("city")}: </dt>
          <dd className="inline">{shipping.city}</dd>
        </div>

        <div>
          <dt className="inline font-medium">{tPurchaseForm("state")}: </dt>
          <dd className="inline">{shipping.state}</dd>
        </div>

        <div>
          <dt className="inline font-medium">{tPurchaseForm("phone")}: </dt>
          <dd className="inline">{shipping.phone}</dd>
        </div>

        <div>
          <dt className="inline font-medium">{tCommon("email")}: </dt>
          <dd className="inline">{shipping.email}</dd>
        </div>
      </dl>
    </section>
  );
};